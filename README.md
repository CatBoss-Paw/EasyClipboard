...
graph TD
    ...
    style C3 fill:#e0f2fe,stroke:#0284c7,color:#000
    style C4 fill:#e8f5e9,stroke:#43a047,color:#000
```

- 你的电脑里一定有几个每天都要打开的文件夹或 Excel 表格，以往总要在“我的电脑”里一层一层点开，费时费力；
- **快速访问区就是你的专属资产百宝箱**：
  - 点击「加文件夹」或直接把文件拖进来，永久固定在侧边；
  - **双击层层下拽钻取**：双击文件夹直接在面板内逐层钻取进入子目录，顶部贴心配备「⬆ 返回上一层」导航按钮；
  - **就地新建分类文件夹**：支持在空白处或素材卡片上右键直接「新建分类文件夹」，就地归类收纳素材；
...
graph TD
    ...
    B --> C["3. 极速双击提取文本全文<br/>在卡片上双击秒级复制"]
    B --> D["4. 右键直通资源管理器<br/>秒级定位高亮 .md 文件"]
    B --> E["5. 无缝联动 Obsidian 笔记<br/>设置中指定路径，实时热重载"]
...
- **看到好的，一秒存起来**：在卡片上右键选择「加入快速…」，存进自己建的分类目录；
- **想用的时候，双击就能用**：点击顶栏切换到「快捷指令」，**双击对应卡片直接提取并复制文档内纯文本全文**，切回文档直接粘贴；
- **资源管理器秒级直通**：右键任意提示词卡片选择「在资源管理器中打开」，直接高亮定位目标 `.md` 文件；点击顶部「🔄 刷新」按钮瞬间完成磁盘对账；
...
```mermaid
graph TD
    A["觉得主工作台占屏幕？<br/>(560x480 完整窗口)"] --> B["点击右上角折叠按钮<br/>🪙 缩成 48x48 微型小方块"]
    B --> C["🖱️ 鼠标按住随心拖拽<br/>贴在屏幕四角或副屏"]
    B --> D["☕ 内存深度修剪收缩<br/>完全不挡视线，极致静默"]
...
- 觉得主窗口占屏幕？
- 点击右上角折叠按钮 `—`，面板立刻缩小为立体文件夹小方块；
- **按住鼠标随便拖**：可以拖到副屏或屏幕四个角落贴边放着，完全不遮挡正在写字或看剧的视线；
- **需要时点一下秒展开**：鼠标点它一下，立刻平滑恢复成完整大工作台！

---

### 📌 本领 7：100% 原地绝对记忆与桌面双击穿透唤醒（稳如泰山，随叫随到）

```mermaid
graph TD
    A["自由摆放与缩放窗口<br/>拖拽到任意屏幕位置"] --> B["实时断电级持久化<br/>写入 shelf_settings.json"]
    B --> C1["⚡ 再次唤出：100% 稳稳停在原地<br/>绝不暴力移回右上角！"]
    B --> C2["🚀 桌面快捷方式双击穿透<br/>Socket握手 + 信号文件双保险"]
    C2 --> D["穿透 Windows 前台焦点防护<br/>毫秒级原地秒弹并获得焦点"]
...
- **100% 原地绝对记忆**：
  - 彻底移除了传统软件唤醒时暴力重置到右上角的坏习惯；
  - 无论您如何拖拽位置、如何用四边把手缩放尺寸，松手即时持久化记忆；下次按 `F9` 唤出，它永远稳稳停在您之前停放的原地！
- **桌面快捷方式双击穿透唤醒**：
  - 软件在后台常驻静默时，**随时在桌面上双击「轻松剪贴板」快捷方式**；
  - 采用独家 `QLocalSocket` 双向 ACK 握手与 `.wake_sig` 150ms 轮询双保险机制，配合 Windows 原生 `SW_SHOW` 与焦点穿透 API，主窗口 100% 瞬间原地弹起并夺得焦点，新进程 0.35 秒内自动退出，绝不卡死，绝不多开！
- **置顶图钉持久化**：点击顶栏「📌」切换置顶状态，重启后依然保留置顶偏好。

---

### ⚙️ 本领 8：清爽贴心的设置中心（素材想留几天你说了算）
...
| **窗口摆放位置记忆** | 每次唤醒都被强制重置到屏幕右上角 | **100% 原地绝对记忆，拖到哪里停在哪里，绝不乱飘** |
| **桌面双击唤醒体验** | 已经运行时双击桌面图标常常没反应或多开卡死 | **原生双保险穿透置顶，双击图标 0.35s 原地秒级弹出主窗口** |
| **高分屏/大屏幕字迹** | 容易文字发虚、泛白模糊、显得廉价 | **硬件级原生逐点物理渲染，字迹像印刷品一样清晰** |
| **电脑内存资源开销** | 很多同类软件开着占 300MB~600MB | **收起时物理内存仅十几 MB，完全不拖慢电脑速度** |

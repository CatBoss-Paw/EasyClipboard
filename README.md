...
```mermaid
graph LR
    A["平时复制 3张图 + 2份文件 + 1段文字"] --> B["轻松剪贴板自动分好类"]
    B -->|鼠标勾选| C["按住底栏「拖出交付」手柄"]
    C -->|直接一甩| D["整包丢进微信/企微发送<br/>(告别来回切换 6 次！)"]
...
- **🔥 核心杀手锏：一键整包拖走（彻底干掉反复切换的痛苦！）**
  - **以前发材料**：要给同事或客户发多张截图、表格和说明，得在聊天窗口和桌面之间**来回切 6 次，复制粘贴 6 次**；
...
```mermaid
graph LR
    A["每天都要找的<br/>工作文件夹 / 核心报表 / 合同"] -->|点「加文件夹」或直接拖入| B["⭐ 永久固定在快速访问栏"]
    B -->|双击条目| C["一秒直通打开对应目录或文件"]
    B -->|按住拖拽| D["直接甩进聊天窗口发送给客户"]
